...
(function (Controller) {
    ...
    Controller.Dashboard.prototype.initType = function () {
        ...
        var me = this;
        var tb = $('#list-type');
        var trs = $(tb.children()[0]).children();
        this.types = [];
        ...
        try {
            var _loop = function _loop() {
                ...
                _this.types.push($(tr).data('value'));
                $(tr).click(function () {
                    me.toggleSelect(tr);
                });
            };
    ...
    Controller.Dashboard.prototype.fillListProvice = function (data) {
        var me = this;
        var table = $('#list-provice');
        table.html("");
        var tbody = $('<tbody></tbody>');
        table.append(tbody);
        if (data.result.length == 0) return;
        data.result.forEach(function (e) {
            var tr = $('<tr data-value="' + e.ma_buu_chinh + '"><td>' + e.name + "</td></tr>");
            tr.click(function () {
                me.toggleSelect(tr);
            });
            tbody.append(tr);
    ...
    Controller.Dashboard.prototype.getSelectedValue = function (id) {
        ...
        try {
            for (var _iterator5 = trs[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                var _tr2 = _step5.value;

                if ($(_tr2).hasClass('selected')) {
                    data.push($(_tr2).data('value'));
                }
            }
        } catch (err) {
    ...
    Controller.Dashboard.prototype.drawReport3 = function (source) {
        ...
        var height = source.result.length * 20 + 20;
